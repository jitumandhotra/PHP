    ...
    public function register_rest_routes() {        
        foreach ($this->routes as $route_key => $route) {
            register_rest_route($this->plugin_name.'/v1', $route['path'], array(
                ...
                'permission_callback' => array($this,'jsun_wp_rest_permission_callback'),
            ));
        }        
            
    }    

    public function jsun_wp_rest_permission_callback($request){
        $authorization_header = $request->get_header('Authorization');
        if (strpos($authorization_header, 'Bearer ') === 0) {
            $api_key = substr($authorization_header, strlen('Bearer '));
        } else {
          return new WP_Error('invalid_authorization_format', __('Invalid Authorization format'), array('status' => 401));
        }
        if ($api_key === get_option('jsun_api_keys', true)) {
            return true; 
        }else{
            return new WP_Error('invalid_api_key', __('Invalid API key'), array('status' => 401));
        }
        die;       
